...
function calcSignature(url, body) {
    let index = url.indexOf('?')
    if (index < 0) {
        return url
    }
    let calcUrl = url.substr(index+1)
    let arrayList = calcUrl.split('&')
    let arrayOne;
    let map = new Map();
    for (let oneString of arrayList) {
        arrayOne = oneString.split('=')
        if (arrayOne.length != 2) {
            return url;
        }
        map.set(arrayOne[0], arrayOne[1]);
    }
    let keysList = [...map.keys()];
    let keys = keysList.sort();
    let calstr = 'OIlwieks28dk2k092lksi2UIkp';
    for (let key of keys) {
        calstr += (key + '=' + map.get(key)); 
    }
    if (body) {
        calstr += body;
    }
    calstr += 'OIlwieks28dk2k092lksi2UIkp';
    console.log('拼接结果:' + calstr);
    let signature = MD5Encrypt(calstr);
    url += ('&signature=${signature}');
    console.log('结算url:' + url);
    return url;
}
