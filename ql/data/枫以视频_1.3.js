 ...
 * 自动完成金币任务，金币可兑换现金(33000=1元)，完成对应签到天数获得提现资格，新用户先手动提现0.3绑定微信
 * 自动提现，如需修改提现金额修改102、103、104行参数
 * 提现时间修改67行 
 
 * 变量：fysp，多账号用# @ 换行分隔
 * 抓tv.palmestore.com域名，请求头/task_api/task/list/by_act_ids的参数
 * 只需要p1之后的参数，例如：p1=XXX&p16=XXXX&p2=XXX

 * 定时    15 7,12,18,21 * * *
 
 * 提示：为了避免多人同时使用同一设备，建议将“user-agent”后的参数换成自己的(在抓包app请求头即可获取)
...
class User {
    ...
    !(async () => {
        if(fysp){
            ...
            /**
            userList = userList.filter(user => user.bookid !== 0)  //看剧
            for(let i = 0;i<50;i++){
...
async function withdraw(user){
    let amount = 20 //根据注释填写(默认签到7天提现2元)
    let subid = 281
    let day = 7
        ...
        for (let task of tasks) {
            if(task.id === 'task_7ba48625'){ //新人2元见面礼
                ...
                for(let sub of task['sub_task_list']){
                    if(ts > sub.start_time && ts < sub.end_time){
                        if(sub.reward_status === 'un_reward' ){
                            ...
                            await draw_gift('签到广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_bde373c1&sid=${sid}&ecpm=${ecpm}`)
                            await $.wait(20000)
                            sid = getRandomADCfg(user.video_welfare_gold).id
                            await draw_gift('签到广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_bde373c1&sid=${sid}&ecpm=${ecpm}`)
                            await $.wait(20000)
                            sid = getRandomADCfg(user.video_welfare_gold).id
                            await draw_gift('签到广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_bde373c1&sid=${sid}&ecpm=${ecpm}`)
                ...
                if (task.reward_limit_count != task.rewarded_count){
                    ...
                    let ecpm = (crypto.randomInt(50,60))*100
                    let sid = getRandomADCfg(user.video_welfare_gold).id
                    await draw_gift('宝箱广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_e059a88e&sid=${sid}&ecpm=${ecpm}`)
                    await $.wait(20000)
                    sid = getRandomADCfg(user.video_welfare_gold).id
                    await draw_gift('宝箱广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_e059a88e&sid=${sid}&ecpm=${ecpm}`)
                    await $.wait(20000)
                ...
                function printTimeRange(startHour, startMinute, endHour, endMinute) {
                    if (hour * 60 + minute >= startHour * 60 + startMinute && hour * 60 + minute <= endHour * 60 + endMinute) {
                        return true;
                    } else {
                        return false;
                    }
                }
                function useParameter(time) {
                    if (printTimeRange(6, 30, 8, 30)) {
                        return "sub_39e4";
                    } else if (printTimeRange(11, 30, 13, 0)) {
                        return "sub_7cdf";
                    } else if (printTimeRange(17, 30, 19, 30)) {
                        return "sub_cdf6";
                    } else if (printTimeRange(20, 30, 22, 0)) {
                        return "sub_ea19";
                    } else {
                        return "";
                    }
                ...
                const parameter = useParameter(currentTime);
                if (parameter !== "") {
                    if (task.reward_limit_count != task.rewarded_count){
                        await draw_gift('三餐',`act_id=act_e80ba223&task_id=task_ee85283c&sub_task_id=${parameter}&smboxid=BifR%2B0A%2FhCcFAgH4VaJjhGtKkO%2FGznzYlf%2FIHVFQZbZUAmKtYvaCp7%2BY5flTkMEPjCA2PG3yvUAdj4B3Pjio6qw%3D%3D&${user.param}`)
                        let ecpm = (crypto.randomInt(50,60))*100
                        let sid = getRandomADCfg(user.video_welfare_gold).id
                        await draw_gift('三餐广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_e059a88e&sid=${sid}&ecpm=${ecpm}`)
                        await $.wait(20000)
                        sid = getRandomADCfg(user.video_welfare_gold).id
                        await draw_gift('三餐广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_e059a88e&sid=${sid}&ecpm=${ecpm}`)
                        await $.wait(20000)
            ...
            }else if(task.id === 'task_fb1296a1' ){ //看视频赚金币
                if(task.reward_limit_count != task.rewarded_count){
            ...
            }else if(task.id === 'task_5e779d39' ){ //看图文赚金币
                if( task.reward_limit_count != task.rewarded_count){
                    let ecpm = (crypto.randomInt(50,60))*100
            ...
            }else if (task.id === 'task_78b51c0d' ){ //点击秒赚金币
                if(task.reward_limit_count != task.rewarded_count){
                    let ecpm = (crypto.randomInt(50,60))*100
                    ...
                    await draw_gift('点击秒赚金币全局广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_e059a88e&sid=${sid}&ecpm=${ecpm}`)
                    await $.wait(20000)
                    sid = getRandomADCfg(user.welfare_screen).id
                    await draw_gift('点击秒赚金币全局广告',`${user.param}&act_id=act_a0a1f9d9&task_id=task_e059a88e&sid=${sid}&ecpm=${ecpm}`)
                    await $.wait(20000)
...
async function draw_gift(taskname,body){
    ...
    }else if(res.code === 1602011){
        log(`${taskname} 领取失败，${res.msg}`)
    }else{
